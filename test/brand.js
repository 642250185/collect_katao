...
const getDetail = async(categroy, url) => {
    try {
        ...
        for(let i = 0; i < len; i++){
            ...
            const nickname = div.find('.col-xs-5').find('.realname').html();
            const publicDate = div.find('.col-xs-5').find('.userName').html();
            const price_rmb = div.find('.col-xs-2').find('.priceBlack').html();
            const memony = div.find('.col-xs-2').find('.text-muted').next().text();
            const carriage = div.find('.col-xs-2').children().html();

            // console.error(carriage);

            console.info(`${nickname}   ${publicDate}   ${price_rmb}   ${memony}   ${imgAlt}   ${imgSrc}`);
        }
    } catch (e) {
        console.error(e);
        return [];
